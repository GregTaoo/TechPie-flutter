...
- The OHOS CI image (`ci/Dockerfile.ohos-buildenv`) still carries no C++ toolchain, although `ohos/entry/src/main/cpp/` is real native code (the bind tunnel's packet I/O): hvigor never compiles it, because the module ships `ohos/entry/libs/arm64-v8a/libecardbind_reader.so` as a committed prebuilt — no `externalNativeOptions` — which `ohos/scripts/build-ecardbind-reader.sh` rebuilds whenever that source changes, with a digest of the source embedded in the .so and asserted by `test/campus_card/architecture/ecard_bind_reader_artifact_test.dart`, so an edit that was never compiled cannot pass unnoticed. Dropping the toolchain is therefore worth its 4.5 GB (clang, its sysroot, the bundled cmake/ninja, and the 1.7 GB BiSheng compiler this build never selects); native code that has to be compiled *at build time* is the one thing that would need the block back. It still drops the previewer's device variant and HMS resource bundle, and the Flutter fork's web SDK, to stay inside a hosted runner's disk budget. Two things that look droppable are not: the fork's 1.9 GB `.git` (`bin/internal/shared.sh` refuses to run without it, and the framework version comes from a git tag) and `openharmony/previewer/common` (the resource compiler dlopens `hms/toolchains/lib/libimage_transcoder_shared.so`, whose RUNPATH points into it; the image build asserts that chain resolves). The image has to be republished (`ci/publish-buildenv.sh`) and its new tag set in `ohos-release.yml` whenever the toolchain it carries changes, or a release keeps building with the old one.
